...
#define MAX_ENTRY_COUNT	58	//ASUS DEBUG
...
static struct pmic_pon_log_entry pm_pon_logs[MAX_ENTRY_COUNT] = {0};
...
int getNextSuccessIndex(struct nvmem_device *nvmem,int addr_start)
{
	...
	addr_start++;
	while(addr_start < MAX_ENTRY_COUNT)
	{
...
static int pmic_pon_log_parse(struct pmic_pon_log_dev *pon_dev)
{
	...
	/* ASUS BSP DEBUG +++ */
	reverse_pm_pon_logs();
	writePowerOnOff2Evtlog(pon_dev->nvmem[0],0);
	/* ASUS BSP DEBUG --- */
