...
uint8_t *frame_report;
uint8_t *d1test_report;
char*	track_report_buffer;
...
*/
	...
	do {
		...
		*/
	} while (!reg_event.fwd && (reg_event.touch || (reg_event.command )));
...
static void read_track_reports(struct snt8100fsr *snt8100fsr) {
	...
	if(snt8100fsr_g->track_report_txt_active) {
...
int enable_track_report_logging(bool enable, int ftype) {
	...
	char log_header[] = "Timestamp, Frame No, Bar, Track ID, Force, Pos0, Pos1, Pos2\n";
        // Alloc the track_report buffer then pass data to user space
	if(track_report_buffer == NULL){
		 track_report_buffer = memory_allocate(2048, GFP_DMA);
		memset(track_report_buffer, 0, 2048);
	}
	...
	if (ftype == 1) {   // bin file
		...
		if (enable) {
                        snt8100fsr_g->track_report_txt_active = true;
			log_track_reports_file_offset = 0;
			// Copy a text header so that the log file can be interpreted
			memcpy(track_report_buffer, log_header, sizeof(log_header)-1);
			...
			snt8100fsr_g->track_report_txt_active = false;
...
static void log_track_report(uint16_t frame,
	...
	if (size > 0) {
                memcpy(&track_report_buffer[log_track_reports_file_offset],log_buf, size);
...
int enable_d1test_logging(struct snt8100fsr *snt8100fsr, int num_frames) {
	...
	*/
	...
	if(d1test_report == NULL){
		// a d1test data is 784 bytes
		d1test_report = memory_allocate(784 * num_frames, GFP_DMA);
	}
...
static void log_d1test(struct snt8100fsr *snt8100fsr) {
	...
	*/
	...
	//if (log_d1test_file != NULL) {
		...
		//		   sc_cmd->length);
	memcpy(&d1test_report[log_d1test_file_offset], (unsigned char *)&sc_cmd->data[0], sc_cmd->length);
...
void frame_report_logging_cleanup(struct snt8100fsr *snt8100fsr)
{
	PRINT_FUNC();

	if (frame_report != NULL) {
		memory_free(frame_report);
		frame_report = NULL;
	}
...
int enable_frame_logging(struct snt8100fsr *snt8100fsr,
	...
	*/
	...
	PRINT_INFO("snt8100fsr->frame_stream_count = %d", snt8100fsr->frame_stream_count);
...
static void log_frame_cmd(struct snt8100fsr *snt8100fsr) {
	...
	if(frame_report == NULL){
		snt8100fsr->total_frame_data_size = frame_size * snt8100fsr->frame_stream_count;
		frame_report = memory_allocate(frame_size*snt8100fsr->frame_stream_count, GFP_DMA);
	}
...
static void log_frame_stream(struct snt8100fsr *snt8100fsr) {
	...
	if (frame_buffer) {
		...
		memcpy(&frame_report[log_frame_file_offset], &frame_buffer[FRAME_LENGTH_WIDTH], frame_size);
		...
		frame_report_logging_cleanup(snt8100fsr);
		goto cleanup;
	}
