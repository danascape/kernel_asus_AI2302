...
static struct odm_modem_rf_vdd_type odm_modem_rf_vdd = { NULL, "modem-rf-vdd-switch", 2800000, 2800000, 0, false};

// referene to q6v5_regulator_enable
static int odm_antenna_power_enable()
{
	int ret = 0;
	struct odm_modem_rf_vdd_type *vreg_antenna = &odm_modem_rf_vdd;

	if (!(vreg_antenna) || !(vreg_antenna->reg)) {
		pr_err("[modem_pas]: odm_antenna_power_enable, (%s) ENODEV.\n", vreg_antenna->name);
		return -ENODEV;
	}

	if (vreg_antenna->enabled) {
		pr_err("[modem_pas]: %s enabled already.\n", vreg_antenna->name);
		return ret;
	}

	ret = regulator_set_voltage(vreg_antenna->reg, vreg_antenna->min_uV, vreg_antenna->max_uV);
	if (ret) {
		pr_err("[modem_pas]: set voltage fail, %s (min_uV:%u, max_uV:%u) ret=%d.\n",
			vreg_antenna->name, vreg_antenna->min_uV, vreg_antenna->max_uV, ret);
		return ret;
	}

	if (vreg_antenna->load_uA > 0) {
		ret = regulator_set_load(vreg_antenna->reg, vreg_antenna->load_uA);
		if (ret < 0) {
			pr_err("[modem_pas]: set load fail, %s load_ua:%u, ret=%d.\n", vreg_antenna->name, vreg_antenna->load_uA, ret);
			return ret;
		}
	}
	...
	if (ret) {
		pr_err("[modem_pas]: %s enable fail, ret=%d.\n", vreg_antenna->name, ret);
		return ret;
	}

	vreg_antenna->enabled = true;
	pr_err("[modem_pas]: %s enabled.\n", vreg_antenna->name);

	return ret;
}

static int odm_antenna_power_disable()
{
	int ret = 0;
	struct odm_modem_rf_vdd_type *vreg_antenna = &odm_modem_rf_vdd;

	if (!(vreg_antenna) || !(vreg_antenna->reg)) {
		pr_err("[modem_pas]: odm_antenna_power_disable, (%s) ENODEV.\n", vreg_antenna->name);
		return -ENODEV;
	}

	if (!(vreg_antenna->enabled)) {
		pr_err("[modem_pas]: %s disable already.\n", vreg_antenna->name);
		return ret;
	}

	ret = regulator_set_voltage(vreg_antenna->reg, 0, vreg_antenna->max_uV);
	if (ret) {
		pr_err("[modem_pas]: set voltage 0 fail, %s ret=%d.\n", vreg_antenna->name, ret);
		return ret;
	}
	...
	if (vreg_antenna->load_uA > 0) {
		...
		if (ret < 0) {
			pr_err("[modem_pas]: set load 0 fail, %s ret=%d.\n", vreg_antenna->name, ret);
			return ret;
		}
	}
	...
	if (ret) {
		pr_err("[modem_pas]: %s disable fail, ret=%d.\n", vreg_antenna->name, ret);
		return ret;
	}

	vreg_antenna->enabled = false;
	pr_err("[modem_pas]: %s disabled.\n", vreg_antenna->name);

	return ret;
}

static int odm_set_antenna_power(int enable)
{
	int ret = 0;

	if (enable == 0)
	{
		pr_err("[modem_pas]: odm_set_antenna_power, antenna_switch_disable_vreg.\n");
		ret = odm_antenna_power_disable();
		if (ret != 0) {
			pr_err("[modem_pas]: odm_set_antenna_power, antenna_switch_disable_vreg failed.\n");
		}
	}
	else
	{
		pr_err("[modem_pas]: odm_set_antenna_power, antenna_switch_enable_vreg.\n");
		ret = odm_antenna_power_enable();
		if (ret != 0)
		{
			pr_err("[modem_pas]: odm_set_antenna_power, antenna_switch_enable_vreg failed.\n");
		}
	}
	return ret;
}

static ssize_t rf_power_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct odm_modem_rf_vdd_type *vreg_antenna = &odm_modem_rf_vdd;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "1")) {
		enable = 1;
		odm_set_antenna_power(enable);
	} else if (sysfs_streq(buf, "0")) {
		enable = 0;
		odm_set_antenna_power(enable);
	} else {
		pr_err("[modem_pas] invalid antenna_power %s\n", buf);
		ret = -EINVAL;
	}
...
static int odm_create_antenna_power_sysfs(struct device *dev, const char *info_name) {
	int ret;
	struct regulator *rf_vdd_reg;

	struct odm_modem_rf_vdd_type *rf_vdd = &odm_modem_rf_vdd;

	if (info_name == NULL || strcmp(info_name, "modem")) {
		return 0;
	}

	//1.get regulator
	rf_vdd_reg = devm_regulator_get(dev, rf_vdd->name);
	if (IS_ERR_OR_NULL(rf_vdd_reg)) {
		ret = PTR_ERR(rf_vdd_reg);
		pr_err("[modem_pas] failed to get %s, rc=%d.\n", rf_vdd->name, ret);
		return ret;
	}
	rf_vdd->reg = rf_vdd_reg;
	pr_err("[modem_pas] %s init ok.\n", rf_vdd->name);
	ret = odm_antenna_power_enable();
	if (ret != 0) {
		pr_err("[modem_pas] power_enable ret=%d.\n", ret);
	}
	//2. create file node
	ret = device_create_file(dev, &dev_attr_rf_power);
	if (ret) {
		pr_err("[modem_pas] sysfs node create failed error:%d\n", ret);
	}
	pr_err("[%s] sysfs node create fw %s\n", rf_vdd->name, info_name);
	return ret;
}

static void odm_remove_rf_power_all(struct device *dev, const char *info_name)
{
	if (info_name != NULL && !strcmp(info_name, "modem")) {
		device_remove_file(dev, &dev_attr_rf_power);
	}
}
...
static int adsp_probe(struct platform_device *pdev)
{
	...
	ret = odm_create_antenna_power_sysfs(adsp->dev, adsp->info_name);
	if (ret) {
		goto remove_rproc_del;
	}
	return 0;
...
static int adsp_remove(struct platform_device *pdev)
{
	...
	odm_remove_rf_power_all(adsp->dev, adsp->info_name);
