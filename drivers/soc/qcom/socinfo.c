...
//ASUS_BSP : check project id to change platform_subtype for select magnetometer json +++
#define PROJECT_VANGOGH_STRING "4"
#define PROJECT_VANGOGH_ESIM_STRING "5"
#define PROJECT_VANGOGH_WO_WIRELESS_STRING "6"

char g_project_id[4];
module_param_string(prj, g_project_id, sizeof(g_project_id), 0600);

enum {
	PROJECT_VANGOGH = 4,
	PROJECT_VANGOGH_ESIM = 5,
	PROJECT_VANGOGH_WO_WIRELESS = 6,
};

const char *hw_platform_subtype_mag[] = {
	[PROJECT_VANGOGH ]= "VANGOGH",
	[PROJECT_VANGOGH_ESIM] = "VANGOGH_ESIM",
	[PROJECT_VANGOGH_WO_WIRELESS] = "VANGOGH_WO_WIRELESS",
};
//ASUS_BSP : check project id to change platform_subtype for select magnetometer json ---
		...
		char *buf)
{
	...
	if (socinfo_get_platform_type() == HW_PLATFORM_QRD) {
		...
		//ASUS_BSP : check project id to change platform_subtype for select magnetometer json +++
		if (!strcmp(PROJECT_VANGOGH_WO_WIRELESS_STRING ,g_project_id)){
				hw_subtype = PROJECT_VANGOGH_WO_WIRELESS;
		}else{
				hw_subtype = PROJECT_VANGOGH;
		}

		printk("ecompass subtype_id: hw_subtype %d, g_project_id %s",hw_subtype, g_project_id);
