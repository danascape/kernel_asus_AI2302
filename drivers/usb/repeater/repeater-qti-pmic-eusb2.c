...
enum usb_path usb_current_path;
...
void repeater_set_current_path(enum usb_path path)
{
	usb_current_path = path;
}EXPORT_SYMBOL_GPL(repeater_set_current_path);
...
struct eusb2_repeater {
	...
	u32			*param_override_seq_BTM;
	u8			param_override_seq_BTM_cnt;
	u32			*param_override_seq_host;
	u8			param_override_seq_host_cnt;
	u32			*param_override_seq_BTM_host;
	u8			param_override_seq_BTM_host_cnt;
};
					...
					u16 reg_offset, u8 mask, u8 write_val)
{
	...
	dev_info(er->ur.dev, "eusb2_repeater_masked_write, reg:0x%02x val:0x%02x\n", reg_offset, write_val);
...
static int eusb2_repeater_init(struct usb_repeater *ur)
{
	...
	if (usb_current_path == PATH_BTM) {
		if(ur->flags & PHY_HOST_MODE){
			dev_info(er->ur.dev, "eUSB2 repeater dtsi params override - BTM-host\n");
			eusb2_repeater_update_seq(er, er->param_override_seq_BTM_host,
				er->param_override_seq_BTM_host_cnt);
		} else {
			dev_info(er->ur.dev, "eUSB2 repeater dtsi params override - BTM\n");
			eusb2_repeater_update_seq(er, er->param_override_seq_BTM,
				er->param_override_seq_BTM_cnt);
		}
	} else if (usb_current_path == PATH_SIDE) {
		if(ur->flags & PHY_HOST_MODE){
			dev_info(er->ur.dev, "eUSB2 repeater dtsi params override - SIDE-host\n");
			eusb2_repeater_update_seq(er, er->param_override_seq_host,
				er->param_override_seq_host_cnt);
		} else {
			dev_info(er->ur.dev, "eUSB2 repeater dtsi params override - SIDE(default)\n");
	...
	eusb2_repeater_reg_read(er, eye_reg, EUSB2_TUNE_USB2_CROSSOVER, 12);

	for (i=0; i < 12; i++) {
		dev_info(er->ur.dev, "reg:0x%02x = 0x%02x\n", EUSB2_TUNE_USB2_CROSSOVER + i, eye_reg[i]);
	}
...
//qcom original dtsi param_override_seq - for SIDE
...
//asus add dtsi param_override_seq_BTM - for BTM
	num_elem = of_property_count_elems_of_size(dev->of_node,
				"qcom,param-override-seq-BTM",
				sizeof(*er->param_override_seq_BTM));
	if (num_elem > 0) {
		if (num_elem % 2) {
			dev_err(dev, "invalid param_override_seq_BTM_len\n");
			ret = -EINVAL;
			goto err_probe;
		}

		er->param_override_seq_BTM_cnt = num_elem;
		er->param_override_seq_BTM = devm_kcalloc(dev,
				er->param_override_seq_BTM_cnt,
				sizeof(*er->param_override_seq_BTM), GFP_KERNEL);
		if (!er->param_override_seq_BTM) {
			ret = -ENOMEM;
			goto err_probe;
		}
		...
		ret = of_property_read_u32_array(dev->of_node,
				"qcom,param-override-seq-BTM",
				er->param_override_seq_BTM,
				er->param_override_seq_BTM_cnt);
		if (ret) {
			dev_err(dev, "qcom,param-override-seq-BTM read failed %d\n",
									ret);
			goto err_probe;
		}
	}
//asus add dtsi param_override_seq_host - for SIDE host
	num_elem = of_property_count_elems_of_size(dev->of_node,
	...
	if (num_elem > 0) {
		...
		er->param_override_seq_host_cnt = num_elem;
...
//asus add dtsi param_override_seq_BTM_host - for BTM host
	num_elem = of_property_count_elems_of_size(dev->of_node,
				"qcom,param-override-seq-BTM-host",
				sizeof(*er->param_override_seq_BTM_host));
	if (num_elem > 0) {
		if (num_elem % 2) {
			dev_err(dev, "invalid param_override_seq_BTM_host_len\n");
			ret = -EINVAL;
			goto err_probe;
		}

		er->param_override_seq_BTM_host_cnt = num_elem;
		er->param_override_seq_BTM_host = devm_kcalloc(dev,
				er->param_override_seq_BTM_host_cnt,
				sizeof(*er->param_override_seq_BTM_host), GFP_KERNEL);
		if (!er->param_override_seq_BTM_host) {
			ret = -ENOMEM;
			goto err_probe;
		}
		...
		ret = of_property_read_u32_array(dev->of_node,
				"qcom,param-override-seq-BTM-host",
				er->param_override_seq_BTM_host,
				er->param_override_seq_BTM_host_cnt);
		if (ret) {
			dev_err(dev, "qcom,param-override-seq-BTM-host read failed %d\n",
									ret);
			goto err_probe;
		}
	}
