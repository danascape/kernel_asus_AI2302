...
#if ASUS_UFS_CHECK
#define UFS_DEBUG 1
#define UFS40_VERSION_MAJOR 4
#define UFS40_VERSION_MINOR 0
#define UFS31_VERSION_MAJOR 3
#define UFS31_VERSION_MINOR 1
u32 ufs_version_major = 0, ufs_version_minor = 0;
...
#if ASUS_UFS_CHECK
	if(ufs_version_major == UFS40_VERSION_MAJOR && ufs_version_minor == UFS40_VERSION_MINOR){
		if ((min_dev_gear == UFS_HS_G4)&&(min_qcom_gear == UFS_HS_G5)){
			min_dev_gear = UFS_HS_G5;
			if(UFS_DEBUG) printk("[ABSP][%s] set min_dev_gear to G5\n", __func__);
		}
	}
#endif
	/*
...
if(hba->saved_uic_err){
    ...
    //if(UFS_DEBUG) printk("[ABSP][%s] set limit_rx_hs_gear to G4\n", __func__);
    //}
}
#endif
...
//printk("%s: asus_ufs_init_check\n", __func__);
	ufs_version_major = dev_major;
	ufs_version_minor = dev_minor;
	if(UFS_DEBUG) printk("[ABSP][%s] ufs_version_major:%d ufs_version_minor:%d\n", __func__, ufs_version_major, ufs_version_minor);
	if(ufs_version_major == UFS40_VERSION_MAJOR && ufs_version_minor == UFS40_VERSION_MINOR){
		if(UFS_DEBUG) printk("[ABSP][%s] set limit_tx_hs_gear to G4\n", __func__);
		host->limit_tx_hs_gear = UFS_HS_G4;
		host->limit_rx_hs_gear = UFS_HS_G4;
	}
#endif
			...
			const char *buf, size_t count)
{
    ...
    if(ufs_version_major == UFS40_VERSION_MAJOR && ufs_version_minor == UFS40_VERSION_MINOR){
		if (v) {
			...
			if(UFS_DEBUG) printk("[ABSP][%s] set limit_tx_hs_gear to G5\n", __func__);
		}
	}
	return count;
}
