...
static int get_test_threshold_mc_sc(void)
{
    ...
    /* init buffer */
    fts_init_buffer(thr->fw_ver_val,thr->basic.fw_ver_val, node_num, false, 0, 0);
...
static void print_thr_mc_sc(void)
{
    ...
    FTS_TEST_DBG("fw_ver_val:%d", thr->basic.fw_ver_val);
    ...
    print_buffer(thr->fw_ver_val, tdata->node.node_num, tdata->node.rx_num);
...
int fts_test_get_testparam_from_ini(char *config_name)
{
    ...
    if (ret != 0) {
        FTS_TEST_ERROR("get ini(request_firmware) fail");
