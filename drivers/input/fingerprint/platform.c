...
ASUS BSP Pin Ctrl --- */
	...
	gf_dev->cs_gpio = of_get_named_gpio(np, "fp-gpio-cs", 0);
	if (gf_dev->cs_gpio < 0) {
		pr_err("falied to get cs gpio!\n");
		return gf_dev->cs_gpio;
	}else{
		printk("[FP][%s] get cs pin success! gf_dev->cs_gpio = %d\n", __func__, gf_dev->cs_gpio);
	}

	r = gpio_request(gf_dev->cs_gpio, "goodix_cs");
	if (r) {
		pr_err("[FP][%s] failed to request cs gpio, r = %d\n", __func__, r);
	}else{
		printk("[FP][%s] cs pin request success!\n", __func__);
	}
	
	r = gpio_direction_output(gf_dev->cs_gpio, 0);
	if (r)
