...
#define BOOT_VIB                "driver/boot_vibration"
...
static int haptics_init_lra_period_config(struct haptics_chip *chip)
{
	...
	dev_dbg(chip->dev, "t_lra_us=%d (%d Hz)\n", t_lra_us, USEC_PER_SEC/t_lra_us);
...
static ssize_t asus_ex_proc_boot_vib_write(struct file *flip,
		const char __user *buf, size_t count, loff_t *ppos)
{
	struct haptics_chip *chip = vib_chip;
	struct haptics_play_info *play = &chip->play;
	char messages[256];
	int rc;

	dev_err(chip->dev, "qcom-hv-haptics.c: kernel boot vibration\n");

	memset(messages, 0, sizeof(messages));

	rc = copy_from_user(messages, buf, count);
	if (rc > 0) {
		rc = -EFAULT;
		goto exit;
	}
...
static struct proc_ops asus_ex_proc_boot_vib_ops = {
	.proc_write  = asus_ex_proc_boot_vib_write,
};

static void haptics_create_sysfs(struct haptics_chip *chip)
{
    proc_create(BOOT_VIB, 0777, NULL, &asus_ex_proc_boot_vib_ops);
}
