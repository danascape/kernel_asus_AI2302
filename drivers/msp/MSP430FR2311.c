...
//			report_motor_event(MOTOR_ROTATE, ROTATE_FINISH);
			printk("[MSP430][%s] INT:roate finish.\n", __func__);
		}
...
int FrqConvertMode(int dir, int angle, int speed) {
	...
	printk("[MSP430][%s] PR2 auto control (Dir:%d, Angle:%d), param=%d, %d %d %d %d %d %d, %d %d %d %d %d %d, %d %d %d %d %d %d", __func__, dir, angle, MotorDefault[0],
...
static void openclose_check_worker(struct work_struct *data)
{
	...
	for(i=0; i<20; i++) {
		...
		printk("[MSP430][%s] z=%d,x=%d,delta_z=%d,delta_x=%d\n",__func__,z,x,delta_z,delta_x);
		delta_z *= delta_z;
		delta_x *= delta_x;
...
static int MSP430FR2311_parse_dt(struct device *dev)
{
	...
	mcu_info->pinctrl_state = pinctrl_lookup_state(mcu_info->pinctrl, "irq_active");
	if(IS_ERR(mcu_info->pinctrl_state)) {
		printk("[MSP430][%s] Cannot find pinctrl state\n", __func__);
		goto err_reset_gpio_request;
	}else {
		printk("[MSP430][%s] select irq_active state\n", __func__);
...
int Zen7_MSP430FR2311_Set_ParamMode(const uint16_t* vals) {
	...
	printk("[MSP430][%s] %d, %d %d %d %d %d %d, %d %d %d %d %d %d, %d %d %d %d %d %d\n", __func__,
...
void report_motor_event(uint8_t OpCode, uint32_t value){
	...
	if(kobject_uevent_env(&((mcu_misc.this_device)->kobj), KOBJ_CHANGE, envp) != 0) {
		...
		//D("[MSP430] kobject_uevent_env ok.\n");
	}
}
