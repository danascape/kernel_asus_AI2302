...
static int ft5652_fw_version_test(struct fts_test *tdata, bool *test_result)
{
	int ret = 0;
	bool result = false;
	struct mc_sc_threshold *thr = &tdata->ic.mc_sc.thr;
    ...
    FTS_TEST_FUNC_ENTER();
    FTS_TEST_SAVE_INFO("\n============ Test Item: F/W Version test\n");
    memset(tdata->buffer, 0, tdata->buffer_length);
    //rawdata = tdata->buffer;

    if (!thr->fw_ver_val ) {
        FTS_TEST_SAVE_ERR("fw_ver_val is null\n");
        ret = -EINVAL;
        goto test_err;
    }
	...
	FTS_DEBUG("IC FW = 0x%02x, FW VAL from INI File = 0x%02x",tdata->fw_ver,thr->fw_ver_val[0]);
	
	if(tdata->fw_ver==thr->fw_ver_val[0])
		result = true;
	else
		result = false;
	...
	if (result) {
			*test_result = true;
			FTS_TEST_SAVE_INFO("------F/W Version Test PASS\n");
	} else {
			*test_result = false;
			FTS_TEST_SAVE_INFO("------F/W Version Test NG\n");
	}
	
		/* save test data */
		/*fts_test_save_data("F/W Differ Test", CODE_M_FWVERSION_TEST,
						   fwver, 0, false, false, *test_result);*/
...
static int start_test_ft5652(void)
{
    ...
    /* F/W Version test */
    if (true == test_item->fw_version_test) {
        ret = ft5652_fw_version_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
