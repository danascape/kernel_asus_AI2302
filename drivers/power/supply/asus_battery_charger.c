...
#define OEM_SET_WLS_FW_UPDATE_COMPLETE          0x2113
#define OEM_OPCODE_READ_BUFFER               	0x10000
#define OEM_OPCODE_WRITE_BUFFER              	0x10001

#define OEM_ASUS_WLC_CHARGE_TYPE_1              200
#define OEM_ASUS_WLC_CHARGE_TYPE_2              201
#define OEM_ASUS_WLC_CHARGE_TYPE_3              202
#define OEM_ASUS_WLC_CHARGE_TYPE_4              203
#define OEM_ASUS_WLC_CHARGE_TYPE_5              204
...
struct iio_channel *phone_skin_thermal_vadc_chan;
...
struct extcon_asus_dev   *wlc_status_extcon;
...
struct delayed_work asus_wlc_thermal_work;
...
struct delayed_work asus_check_cep_work;
...
struct delayed_work asus_phone_skin_thermal_work;
...
struct ADSP_ChargerPD_Info {
    ...
    int     wlc_iout_value;
    int     wlc_temp_value;
    int     wlc_vout_set_value;
    int     wlc_opmode_value;
    int     wlc_op_freq_value;
    int     wlc_ping_freq_value;
    int     wlc_ovp_value;
    int     wlc_epp_w_value;
    int     set_wlcthermal;
    int     wlc_charge_type;
...
enum phone_skin_thermal_ac_state g_last_skin_thermal_ac_state = PHONE_SKIN_THERMAL_NONE;
int g_wlc_type_switch = -1;
bool g_fw_updating_flag = false;

extern void qti_charge_notify_device_charge(void);
extern void qti_charge_notify_device_not_charge(void);
extern void qti_charge_notify_device_wls_charge(void);
extern void qti_charge_notify_device_wls_uncharge(void);
...
void asus_phone_skin_thermal_worker(struct work_struct *work)
{
    int rc;
    int skin_temp;
    u32 tmp = 0;
    ...
    if(g_Charger_mode)
    {
        rc = iio_read_channel_processed(phone_skin_thermal_vadc_chan, &skin_temp);
        if (rc < 0)
            CHG_DBG_E("%s: iio_read_channel_processed fail\n", __func__);
        else
            CHG_DBG_E("%s: skin_temp = %d\n", __func__, skin_temp);
        if(skin_temp < 36500) //<36500
        {
            tmp = PHONE_SKIN_THERMAL_NO_LIMITE;
            g_last_skin_thermal_ac_state = tmp;
            goto out;
        }

        if(skin_temp < 38000 && skin_temp >= 36500) // <38000, >=36500
        {
            if(g_last_skin_thermal_ac_state == PHONE_SKIN_THERMAL_LIMITE_1)
                tmp = PHONE_SKIN_THERMAL_LIMITE_1;
            else 
                tmp = PHONE_SKIN_THERMAL_NO_LIMITE;
            g_last_skin_thermal_ac_state = tmp;
            goto out;
        ...
        if(skin_temp >= 38000 && skin_temp < 40500)
        {
            tmp = PHONE_SKIN_THERMAL_LIMITE_1;
            g_last_skin_thermal_ac_state = tmp;
            goto out;
        ...
        if(skin_temp >= 40500 && skin_temp <42000)
        {
            if(g_last_skin_thermal_ac_state == PHONE_SKIN_THERMAL_LIMITE_2)
                tmp = PHONE_SKIN_THERMAL_LIMITE_2;
            else 
                tmp = PHONE_SKIN_THERMAL_LIMITE_1;
            g_last_skin_thermal_ac_state = tmp;
            goto out;
        ...
        if(skin_temp >= 42000 && skin_temp <45000)
        {
            tmp = PHONE_SKIN_THERMAL_LIMITE_2;
            g_last_skin_thermal_ac_state = tmp;
            goto out;
        ...
        if(skin_temp >= 45000 && skin_temp <50000)
        {
            if(g_last_skin_thermal_ac_state == PHONE_SKIN_THERMAL_LIMITE_3)
                tmp = PHONE_SKIN_THERMAL_LIMITE_3;
            else 
                tmp = PHONE_SKIN_THERMAL_LIMITE_2;
            g_last_skin_thermal_ac_state = tmp;
            goto out;
    ...
    out:
        CHG_DBG_E("%s. set BATTMAN_OEM_SKIN_THERMAL_AC_LIMIT : %d", __func__, tmp);
        rc = oem_prop_write(BATTMAN_OEM_SKIN_THERMAL_AC_LIMIT, &tmp, 1);
        if (rc < 0) {
            pr_err("Failed to set BATTMAN_OEM_SKIN_THERMAL_AC_LIMIT rc=%d\n", rc);
        }
        schedule_delayed_work(&asus_phone_skin_thermal_work, msecs_to_jiffies(3000));
    }
...
void monitor_charging_enable(void)
{
    ...
    pr_err("Enter %s, g_Charger_mode = %d\n", __func__, g_Charger_mode);
    if (g_Charger_mode) {
...
void asus_check_cep_worker(struct work_struct *work){
    int rc;
    int cep;
    rc = oem_prop_read(BATTMAN_OEM_WLC_CEP_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_CEP_VALUE rc=%d\n", rc);
        return;
    }

    cep = ChgPD_Info.wlc_cep_val;
    if(cep >= 10 && cep <= 50 && wlc_status_extcon_last_value != 1 && !g_fw_updating_flag){
        asus_extcon_set_state_sync(wlc_status_extcon, 1);
        wlc_status_extcon_last_value = 1;
    }
    else{
        if(wlc_status_extcon_last_value != 0 && !g_fw_updating_flag) {
            asus_extcon_set_state_sync(wlc_status_extcon, 0);
            wlc_status_extcon_last_value = 0;
        }
    }
    schedule_delayed_work(&asus_check_cep_work, 5 * HZ);
    return;
}

static void charger_panel_notifier_callback(enum panel_event_notifier_tag tag,
...
void asus_notify_wlc_type(void){
    int rc;
    int type;

    pr_err("Enter %s", __func__);
    rc = oem_prop_read(BATTMAN_OEM_WLC_CHARGE_TYPE, 1);
    if(rc < 0){
        pr_err("Failed to get BATTMAN_OEM_WLC_CHARGE_TYPE rc=%d\n", rc);
        return;
    }
    type = ChgPD_Info.wlc_charge_type;
    if(g_wlc_type_switch == -1){
        switch(type){
            case 1:
                pr_err("Set quickchg_extcon val %d", OEM_ASUS_WLC_CHARGE_TYPE_1);
                asus_extcon_set_state_sync(quickchg_extcon, OEM_ASUS_WLC_CHARGE_TYPE_1);
                break;
            case 2:
                pr_err("Set quickchg_extcon val %d", OEM_ASUS_WLC_CHARGE_TYPE_2);
                asus_extcon_set_state_sync(quickchg_extcon, OEM_ASUS_WLC_CHARGE_TYPE_2);
                break;
            case 3:
                pr_err("Set quickchg_extcon val %d", OEM_ASUS_WLC_CHARGE_TYPE_2);
                asus_extcon_set_state_sync(quickchg_extcon, OEM_ASUS_WLC_CHARGE_TYPE_2);
                break;
            case 4:
                pr_err("Set quickchg_extcon val %d", OEM_ASUS_WLC_CHARGE_TYPE_3);
                asus_extcon_set_state_sync(quickchg_extcon, OEM_ASUS_WLC_CHARGE_TYPE_3);
                break;
            case 5:
                pr_err("Set quickchg_extcon val %d", OEM_ASUS_WLC_CHARGE_TYPE_3);
                asus_extcon_set_state_sync(quickchg_extcon, OEM_ASUS_WLC_CHARGE_TYPE_3);
                break;
            default:
                pr_err("Error wlc_charge_type code!");
                break;
        }
    }
    else{
        switch(type){
            case 1:
                pr_err("Set quickchg_extcon val %d", g_wlc_type_switch);
                asus_extcon_set_state_sync(quickchg_extcon, g_wlc_type_switch);
                break;
            case 2:
                pr_err("Set quickchg_extcon val %d", g_wlc_type_switch + 1);
                asus_extcon_set_state_sync(quickchg_extcon, g_wlc_type_switch + 1);
                break;
            case 3:
                pr_err("Set quickchg_extcon val %d", g_wlc_type_switch + 2);
                asus_extcon_set_state_sync(quickchg_extcon, g_wlc_type_switch + 2);
                break;
            case 4:
                pr_err("Set quickchg_extcon val %d", g_wlc_type_switch + 3);
                asus_extcon_set_state_sync(quickchg_extcon, g_wlc_type_switch + 3);
                break;
            case 5:
                pr_err("Set quickchg_extcon val %d", g_wlc_type_switch + 4);
                asus_extcon_set_state_sync(quickchg_extcon, g_wlc_type_switch + 4);
                break;
            default:
                pr_err("Error wlc_charge_type code!");
                break;
...
EXPORT_SYMBOL(asus_notify_wlc_type);
...
void RegisterDRMCallback(void)
{
    ...
    if (active_panel) {
        ...
        cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
                ...
                &charger_panel_notifier_callback, &ts_data);
        if (!cookie) {
...
static void asus_jeita_cc_worker(struct work_struct *dat){
	...
	__pm_wakeup_event(jeita_prechg_ws, 15 * 1000);
        schedule_delayed_work(&asus_jeita_cc_work, 1 * HZ);
    }else{
        if(g_Charger_mode){
            printk(KERN_ERR "[BAT][CHG]%s g_Charger_mode is true 15 s", __func__);
            schedule_delayed_work(&asus_jeita_cc_work, 15 * HZ);
        }else {
            printk(KERN_ERR "[BAT][CHG]%s g_Charger_mode is false 5 s", __func__);
...
static int file_op(const char *filename, loff_t offset, char *buf, int length, int operation)
{
    ...
    pr_err("Enter %s, filename = %s", __func__, filename);
...
static ssize_t bat_safety_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    memset(messages, 0, sizeof(messages));
     ...
     //fix ++
    pr_err("%s wirte %s", __func__, file->f_path.dentry->d_iname);
    if(!strcmp(file->f_path.dentry->d_iname, "bat_bs_wr")) {
        ...
        CHG_DBG_E("[BAT][CHG]%s, data_buf: magic:0x%x, cycle_count:%d, high_temp_total_time:%d, high_temp_vol_time:%d, high_vol_total_time:%d, reload_condition:%d, battery_total_time:%d",
                __func__,data_buf.magic, data_buf.cycle_count, data_buf.high_temp_total_time, data_buf.high_temp_vol_time,
                data_buf.high_vol_total_time, data_buf.reload_condition, data_buf.battery_total_time);
...
static int init_batt_cycle_count_data(void)
{
    ...
    CHG_DBG("[BAT][CHG]%s first cycle_count=%d;reload_condition=%d;high_temp_total_time=%lu;high_temp_vol_time=%lu;high_vol_total_time=%lu;battery_total_time=%lu,magic=%d\n",
                __func__, buf.cycle_count,buf.reload_condition, buf.high_temp_total_time,buf.high_temp_vol_time,buf.high_vol_total_time,buf.battery_total_time,buf.magic);
    ...
    if(buf.magic != CYCLE_COUNT_DATA_MAGIC) {
        ...
        CHG_DBG("[BAT][CHG]%s second cycle_count=%d;reload_condition=%d;high_temp_total_time=%lu;high_temp_vol_time=%lu;high_vol_total_time=%lu;battery_total_time=%lu\n",
...
DONE:
    if(ASUS_media_en == 0){
        ...
        pr_err("%s ASUS_media_en set %d, do nothing!", __func__,ASUS_media_en);
    }
...
static void create_batt_cycle_count_proc_file(void)
{
    ...
    if (!asus_batt_cycle_count_proc_file)
        CHG_DBG("[BAT][CHG]%s asus_batt_cycle_count_proc_file create failed!\n", __func__);
    ...
    if(!asus_batt_batt_safety_proc_file)
        CHG_DBG("[BAT][CHG]%s asus_batt_batt_safety_proc_file create failed!\n", __func__);
    ...
    pr_err("%s Batt_bs.read = %d, Batt_bs.write = %d, Batt_bs.message = %s\n",__func__, Batt_bs.read, Batt_bs.message);
...
void asus_wlc_thermal_worker(struct work_struct *work)
{
    int rc;
    u32 tmp;

    tmp = ChgPD_Info.set_wlcthermal;
    CHG_DBG("%s. set BATTMAN_OEM_WLC_THERMAL : %d", __func__, tmp);
    rc = oem_prop_write(BATTMAN_OEM_WLC_THERMAL, &tmp, 1);
    if (rc < 0) {
        pr_err("Failed to set BATTMAN_OEM_WLC_THERMAL rc=%d\n", rc);
    }
}
...
void asus_monitor_start(int status){
    ...
    cancel_delayed_work_sync(&asus_check_cep_work);
	cancel_delayed_work_sync(&asus_slow_charging_work);
    if(!g_fw_updating_flag)
        asus_extcon_set_state_sync(wlc_status_extcon, 0);
    qti_charge_notify_device_wls_uncharge();
	schedule_delayed_work(&asus_slow_charging_work, 0 * HZ);
    cancel_delayed_work_sync(&asus_wlc_thermal_work);
...
void asus_start_check_cep(int wls_status){
    if(!g_asuslib_init) return;
    if (wls_status == asus_wls_online) return;
    schedule_delayed_work(&asus_check_cep_work, 5 * HZ);
}
EXPORT_SYMBOL(asus_start_check_cep);

void asus_stop_check_cep(void){
    cancel_delayed_work_sync(&asus_check_cep_work);
    if(wlc_status_extcon_last_value != 0 && !g_fw_updating_flag){
        asus_extcon_set_state_sync(wlc_status_extcon, 0);
        wlc_status_extcon_last_value = 0;
...
EXPORT_SYMBOL(asus_stop_check_cep);

static ssize_t wlc_status_extcon_debug_store(struct class *c,
                    ...
                    const char *buf, size_t count)
{
    ...
    CHG_DBG("%s. tmp is : 0x%x", __func__, tmp);

    asus_extcon_set_state_sync(wlc_status_extcon, (int)tmp);

    return count;
}
static CLASS_ATTR_WO(wlc_status_extcon_debug);

static ssize_t wlc_cep_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    return scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_cep_val);
}
static CLASS_ATTR_RO(wlc_cep_value);

static ssize_t wlc_vrect_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_VRECT_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_VRECT_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_vrect_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_vrect_value);

static ssize_t wlc_vout_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_VOUT_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_VOUT_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_vout_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_vout_value);

static ssize_t wlc_iout_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_IOUT_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_IOUT_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_iout_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_iout_value);

static ssize_t set_ASUS_media_store(struct class *c, struct class_attribute *attr,
					const char *buf, size_t count)
{
	int tmp;
	...
	tmp = simple_strtol(buf, NULL, 10);
	ASUS_media_en = tmp;
	CHG_DBG_E("%s Set ASUS_media_en to %d\n", __func__, ASUS_media_en);
	return count;
}
...
static ssize_t wlc_temp_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_TEMP_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_TEMP_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_temp_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_temp_value);

static ssize_t wlc_vout_set_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_VOUT_SET_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_VOUT_SET_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_vout_set_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_vout_set_value);

static ssize_t wlc_opmode_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_OPMODE_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_OPMODE_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_opmode_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_opmode_value);

static ssize_t wlc_op_freq_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_OP_FREQ_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_OP_FREQ_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_op_freq_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_op_freq_value);

static ssize_t wlc_ping_freq_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_PING_FREQ_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_PING_FREQ_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_ping_freq_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_ping_freq_value);

static ssize_t wlc_ovp_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_OVP_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_OVP_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_ovp_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_ovp_value);

static ssize_t wlc_epp_w_value_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_EPP_W_VALUE, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_EPP_W_VALUE rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_epp_w_value);
    return rc;
}
static CLASS_ATTR_RO(wlc_epp_w_value);

static bool wlc_nu1628_fw_check_version(void)
{
	struct psy_state *pst;
	int rc;
	u32 target_version = 17;
	u32 current_version;
	...
	current_version = g_bcdev->wls_fw_version;
	pr_err("FW current version=%d\n", current_version);
...
static ssize_t wlc_nu1628_fw_update_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;
    bool check_version_flag;
    u32 tmp = 1;

    check_version_flag = wlc_nu1628_fw_check_version();

    if(!check_version_flag){
        CHG_DBG("Current version is: %d, no need to update", g_bcdev->wls_fw_version);
        asus_extcon_set_state_sync(wlc_status_extcon, 2);
        return scnprintf(buf, PAGE_SIZE, "%#x\n", g_bcdev->wls_fw_version);
    }
    g_fw_updating_flag = true;
    asus_extcon_set_state_sync(wlc_status_extcon, 3);
    rc = oem_prop_write(BATTMAN_OEM_WLC_FW_UPDATE, &tmp, 1);
    if (rc < 0) {
        pr_err("Failed to set BATTMAN_OEM_WLC_FW_UPDATE rc=%d\n", rc);
        return rc;
    }

    return scnprintf(buf, PAGE_SIZE, "%#x\n", g_bcdev->wls_fw_version);
}
static CLASS_ATTR_RO(wlc_nu1628_fw_update);

static ssize_t wlc_ic_status_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;

    rc = oem_prop_read(BATTMAN_OEM_WLC_IC_STATUS, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_WLC_IC_STATUS rc=%d\n", rc);
        return rc;
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.wlc_ic_status);
    return rc;
}
static CLASS_ATTR_RO(wlc_ic_status);

static ssize_t wlc_nu1628_force_update_store(struct class *c,
                    struct class_attribute *attr,
                    const char *buf, size_t count)
{
    int rc;
    ...
    CHG_DBG("%s. BATTMAN_OEM_WLC_FW_UPDATE : 0x%x", __func__, tmp);
...
static CLASS_ATTR_WO(wlc_nu1628_force_update);
...
static ssize_t wlc_charge_type_debug_store(struct class *c,
                    struct class_attribute *attr,
                    const char *buf, size_t count){
    u32 tmp;
    tmp = simple_strtol(buf, NULL, 10);
    CHG_DBG("%s. tmp is : 0x%x", __func__, tmp);
    asus_extcon_set_state_sync(quickchg_extcon, (int)tmp);
    return count;
}
static CLASS_ATTR_WO(wlc_charge_type_debug);

static ssize_t wlc_charge_type_switch_store(struct class *c,
                    struct class_attribute *attr,
                    const char *buf, size_t count){
    u32 tmp;
    tmp = simple_strtol(buf, NULL, 10);
    g_wlc_type_switch = (int)tmp;
    CHG_DBG("%s. tmp is : 0x%x", __func__, tmp);
    return count;
}
static CLASS_ATTR_WO(wlc_charge_type_switch);
                    ...
                    const char *buf, size_t count)
{
    ...
    if (tmp == 0) {
	    ...
	    g_Charger_mode = 1;
        schedule_delayed_work(&asus_phone_skin_thermal_work, msecs_to_jiffies(0));
    }
...
static ssize_t write_nu1628_register_store(struct class *c,
                    struct class_attribute *attr,
                    const char *buf, size_t count)
{
    int tmp[2];
    ...
    rc = oem_prop_write(BATTMAN_OEM_Write_NU1628_Register, tmp, 2);
    if (rc < 0) {
            pr_err("Failed to set BATTMAN_OEM_Write_NU1628_Register rc=%d\n", rc);
            return rc;
    }
    return count;
}

static CLASS_ATTR_WO(write_nu1628_register);

static ssize_t read_pm8350b_register_store(struct class *c,
                    struct class_attribute *attr,
                    const char *buf, size_t count)
{
    int tmp;
    ...
    rc = oem_prop_write(BATTMAN_OEM_Read_NU1628_Register, &tmp, 1);
    if (rc < 0) {
            pr_err("Failed to set BATTMAN_OEM_Read_NU1628_Register rc=%d\n", rc);
            return rc;
    }
    return count;
}

int nu1628_reg_val = 0;

static ssize_t read_pm8350b_register_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    int rc;
    int tem_val = 0;

    rc = oem_prop_read(BATTMAN_OEM_Read_NU1628_Register, 1);
    if (rc < 0) {
        pr_err("Failed to get BATTMAN_OEM_Read_NU1628_Register rc=%d\n", rc);
        return rc;
    }

    tem_val = nu1628_reg_val;
    nu1628_reg_val = 0;

    return scnprintf(buf, PAGE_SIZE, "0x%2x\n", tem_val);
}
static CLASS_ATTR_RW(read_pm8350b_register);
...
static CLASS_ATTR_RW(pm8350b_icl);
 ...
 static ssize_t set_wlcthermal_store(struct class *c,
                      struct class_attribute *attr,
                      const char *buf, size_t count)
{
    int mask;
    mask = simple_strtol(buf, NULL, 16);
    ChgPD_Info.set_wlcthermal = mask;
    CHG_DBG_E("%s set_wlcthermal =%d", __func__, mask);

    schedule_delayed_work(&asus_wlc_thermal_work, 0);
    return count;
}

static ssize_t set_wlcthermal_show(struct class *c,
                    struct class_attribute *attr, char *buf)
{
    return scnprintf(buf, PAGE_SIZE, "%d\n", ChgPD_Info.set_wlcthermal);
}
static CLASS_ATTR_RW(set_wlcthermal);
				...
				size_t len)
{
    ...
    struct oem_enable_change_msg *wlc_fw_update_success_msg;
    ...
    switch(hdr->opcode) {
		...
		pr_err("%s. OEM_SET_OTG_WA enable is %d \n", __func__,enable_change_msg->enable);
                if (gpio_is_valid(OVP_LOAD_SWITCH_GPIO)) {
                    rc = gpio_direction_output(OVP_LOAD_SWITCH_GPIO, enable_change_msg->enable);
                    if (rc)
                        pr_err("%s. Failed to control OVP_LOAD_SWITCH_GPIO\n", __func__);
                } else {
                    pr_err("%s. OVP_LOAD_SWITCH_GPIO is invalid\n", __func__);
		}
                ...
                if (gpio_is_valid(OTG_LOAD_SWITCH_GPIO)) {
                    ...
                    pr_err("%s. OTG_LOAD_SWITCH_GPIO is invalid\n", __func__);
		}
            } else {
	...
	case OEM_SET_CHARGER_TYPE_CHANGE:
		...
		break;
    case OEM_SET_WLS_FW_UPDATE_COMPLETE:
        if (len == sizeof(*wlc_fw_update_success_msg)) {
                wlc_fw_update_success_msg = data;
                pr_err("%s. OEM_SET_WLS_FW_UPDATE_COMPLETE enable is %d \n", __func__,wlc_fw_update_success_msg->enable);
                asus_extcon_set_state_sync(wlc_status_extcon, 4);
                g_fw_updating_flag = false;
        }
        break;
				...
				size_t len)
{
	...
	case OEM_OPCODE_READ_BUFFER:
		if (len == sizeof(*oem_read_buffer_resp_msg)) {
			...
			case BATTMAN_OEM_FG_SoC:
				...
				ack_set = true;
				break;
            case BATTMAN_OEM_WLC_IC_STATUS:
                CHG_DBG("%s BATTMAN_OEM_WLC_IC_STATUS successfully\n", __func__);
                ChgPD_Info.wlc_ic_status = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_CEP_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_CEP_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_cep_val = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_CHARGE_TYPE:
                CHG_DBG("%s BATTMAN_OEM_WLC_CHARGE_TYPE successfully\n", __func__);
                ChgPD_Info.wlc_charge_type = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
			case BATTMAN_OEM_Read_NU1628_Register:
				CHG_DBG("%s BATTMAN_OEM_Read_NU1628_Register successfully\n", __func__);
				nu1628_reg_val = oem_read_buffer_resp_msg->data_buffer[0];
				ack_set = true;
				break;
            case BATTMAN_OEM_WLC_VRECT_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_VRECT_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_vrect_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;		
            case BATTMAN_OEM_WLC_VOUT_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_VOUT_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_vout_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_IOUT_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_IOUT_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_iout_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_TEMP_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_TEMP_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_temp_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_VOUT_SET_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_VOUT_SET_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_vout_set_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_OPMODE_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_OPMODE_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_opmode_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_OP_FREQ_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_OP_FREQ_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_op_freq_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_PING_FREQ_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_PING_FREQ_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_ping_freq_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_OVP_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_OVP_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_ovp_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;
            case BATTMAN_OEM_WLC_EPP_W_VALUE:
                CHG_DBG("%s BATTMAN_OEM_WLC_EPP_W_VALUE successfully\n", __func__);
                ChgPD_Info.wlc_epp_w_value = oem_read_buffer_resp_msg->data_buffer[0];
                ack_set = true;
                break;					
			default:
				ack_set = true;
	...
	case OEM_OPCODE_WRITE_BUFFER:
		if (len == sizeof(*oem_write_buffer_resp_msg)) {
            ...
            case BATTMAN_OEM_WLC_THERMAL:
            case BATTMAN_OEM_FV:
			case BATTMAN_OEM_CHG_Disable_Jeita:
			case BATTMAN_OEM_WLC_FW_UPDATE:
			case BATTMAN_OEM_Write_NU1628_Register:
            case BATTMAN_OEM_Read_NU1628_Register:
            case BATTMAN_OEM_SKIN_THERMAL_AC_LIMIT:
			...
			pr_err("Incorrect response length %zu for OEM_OPCODE_WRITE_BUFFER\n", len);
		}
		break;
	default:
...
static struct attribute *asuslib_class_attrs[] = {
    ...
    &class_attr_set_wlcthermal.attr,
	...
	&class_attr_wlc_ic_status.attr,
	&class_attr_wlc_nu1628_fw_update.attr,
	&class_attr_wlc_status_extcon_debug.attr,
	&class_attr_wlc_cep_value.attr,
	&class_attr_wlc_nu1628_force_update.attr,
	&class_attr_write_nu1628_register.attr,
	&class_attr_read_pm8350b_register.attr,
    &class_attr_wlc_vrect_value.attr,
    &class_attr_wlc_vout_value.attr,
    &class_attr_wlc_iout_value.attr,
    &class_attr_wlc_temp_value.attr,
    &class_attr_wlc_vout_set_value.attr,
    &class_attr_wlc_opmode_value.attr,
    &class_attr_wlc_op_freq_value.attr,
    &class_attr_wlc_ping_freq_value.attr,
    &class_attr_wlc_ovp_value.attr,
    &class_attr_wlc_epp_w_value.attr,
    &class_attr_set_ASUS_media.attr,
    &class_attr_wlc_charge_type_debug.attr,
    &class_attr_wlc_charge_type_switch.attr,
	NULL,
};
...
int asuslib_init(void) {
    ...
    //[+++]Register phone_skin_thermal
    phone_skin_thermal_vadc_chan = devm_iio_channel_get(g_bcdev->dev, "pm8350b_thm6_amux_gpio10");
    if (IS_ERR_OR_NULL(phone_skin_thermal_vadc_chan)) {
        CHG_DBG_E("%s: phone_skin_thermal iio_channel_get fail: %d\n", __func__, phone_skin_thermal_vadc_chan);
    }else{
        INIT_DELAYED_WORK(&asus_phone_skin_thermal_work, asus_phone_skin_thermal_worker);
    }
	...
	// Register the class node
        ...
        OVP_LOAD_SWITCH_GPIO = of_get_named_gpio(g_bcdev->dev->of_node, "OVP_LOAD_SWITCH", 0);
        rc = gpio_request(OVP_LOAD_SWITCH_GPIO, "OVP_LOAD_SWITCH");
        if (rc) {
            pr_err("%s: Failed to initalize the OVP_LOAD_SWITCH\n", __func__);
            return -1;
        }

        if (gpio_is_valid(OVP_LOAD_SWITCH_GPIO)) {
            rc = gpio_direction_output(OVP_LOAD_SWITCH_GPIO, 0);
            if (rc)
                pr_err("%s. Failed to control OVP_LOAD_SWITCH_GPIO\n", __func__);
        } else {
            pr_err("%s. OVP_LOAD_SWITCH_GPIO is invalid\n", __func__);
	...
	jeita_prechg_ws = wakeup_source_register(g_bcdev->dev, "jeita_prechg_wake_lock");
	...
	//[+++]Register the extcon for WLC Notify
	wlc_status_extcon = extcon_asus_dev_allocate();
	if (IS_ERR(wlc_status_extcon)) {
		rc = PTR_ERR(wlc_status_extcon);
		printk(KERN_ERR "[BAT][CHG] failed to allocate ASUS wlc status extcon device rc=%d\n", rc);
	}
	wlc_status_extcon->name = "wlc_status";
	rc = extcon_asus_dev_register(wlc_status_extcon);
	if (rc < 0)
		printk(KERN_ERR "[BAT][CHG] failed to register ASUS wlc status extcon device rc=%d\n", rc);
    ...
    //[+++]Register the extcon for thermal alert
    usb_conn_temp_vadc_chan = devm_iio_channel_get(g_bcdev->dev, "pm8350b_usb_amux_thm4");
    if (IS_ERR_OR_NULL(usb_conn_temp_vadc_chan)) {
        CHG_DBG_E("%s: usb_conn_temp iio_channel_get fail: %d\n", __func__, usb_conn_temp_vadc_chan);
    }else{
	...
	INIT_DELAYED_WORK(&asus_18W_workaround_work, asus_18W_workaround_worker);
        ...
        INIT_DELAYED_WORK(&asus_wlc_thermal_work, asus_wlc_thermal_worker);
    ...
    //check cep work
    INIT_DELAYED_WORK(&asus_check_cep_work, asus_check_cep_worker);
...
int asuslib_deinit(void) {
	...
	wakeup_source_unregister(jeita_prechg_ws);
