...
//	}
	...
	if(ASUS_HALL2_CHANGE == 1){
		hall2_info("%s Hall2 sensor changed here !", __func__);
		hall_sensor_dev->status = !hall_sensor_dev->status;
...
static ssize_t show_hall_sensor_change(struct device *dev,struct device_attribute *attr, char *buf)
{
	if(!hall_sensor_dev)
		return sprintf(buf, "Hall2 sensor does not exist!\n");
	return sprintf(buf, "%d\n",hall_sensor_dev->change);
}
static ssize_t store_hall_sensor_change(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int request;
	sscanf(buf, "%du", &request);
    	if (request == 8){
		hall2_info("%s hall2 sensor get change request !",__func__);
		ASUS_HALL2_CHANGE=1;
		queue_delayed_work(hall_sensor_wq, &hall_sensor_dev->hall_sensor_work, 0);
		__pm_stay_awake(hall_sensor_dev->wake_src);
	} else {
		hall2_info("%s failed to get change request because change = %d ",__func__,request);
	}
	hall2_info("%s getchange = %d\n", __func__, hall_sensor_dev->change);
	return count;
}
...
static DEVICE_ATTR(change, 0664,show_hall_sensor_change, store_hall_sensor_change);
...
static void __exit hall_sensor_exit(void)
{
	...
	sysfs_remove_group(&g_pdev->dev.kobj, &hall_sensor_group);
