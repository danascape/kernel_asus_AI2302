...
extern void asus_start_check_cep(int wls_status);
extern void asus_stop_check_cep(void);
...
extern asus_notify_wlc_type(void);
...
enum qti_power_supply_wls_charged_notify {
	WLS_ORIGIN_NOTIFY,
	WLS_CHARGE_NOTIFY,
	WLS_UNCHARGE_NOTIFY,
};
enum qti_power_supply_wls_charged_notify \
	last_qti_power_supply_wls_charged_notify = WLS_ORIGIN_NOTIFY;
		...
		union power_supply_propval *pval)
{
	...
	if(prop == POWER_SUPPLY_PROP_ONLINE && pval->intval == 1){
		asus_start_check_cep(pval -> intval);
		asus_notify_wlc_type();
		if(last_qti_power_supply_wls_charged_notify != WLS_CHARGE_NOTIFY){
			qti_charge_notify_device_wls_charge();
			last_qti_power_supply_wls_charged_notify = WLS_CHARGE_NOTIFY;
		}
	}
	...
	if(prop == POWER_SUPPLY_PROP_ONLINE && pval->intval != 1){
		...
		if(last_qti_power_supply_wls_charged_notify == WLS_CHARGE_NOTIFY){
			...
			last_qti_power_supply_wls_charged_notify = WLS_UNCHARGE_NOTIFY;
		}
	}
