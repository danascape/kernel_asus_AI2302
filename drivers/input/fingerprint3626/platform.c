...
int gf_power_on(struct gf_dev *gf_dev)
{
	...
	if (regulator_count_voltages(gf_dev->vcc) > 0) {
		...
		if (ret) {
			pr_err("[GF][%s] pm8010m ldo5 regulator set_vtg failed, ret=%d\n", __func__, ret);
			regulator_put(gf_dev->vcc);
		}
	}
	ret = regulator_enable(gf_dev->vcc);
	if (ret) {
		pr_err("[GF][%s] enable pm8010m ldo5 regulator failed, ret=%d\n", __func__, ret);
	} else {		
		pr_info("[GF][%s] enable pm8010m ldo5 regulator successfully!\n", __func__);
	}
...
int gf_power_off(struct gf_dev *gf_dev)
{
	...
	//	gf_dev->power_enabled = 0;
		...
		if (ret) {
			pr_err("[GF][%s] disable pm8010m ldo5 regulator failed, ret=%d\n", __func__, ret);
		} else {
			pr_info("[GF][%s] disable pm8010m ldo5 regulator successfully!\n", __func__);
		}
	if (regulator_count_voltages(gf_dev->vcc) > 0)
